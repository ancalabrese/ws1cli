...
func generateLeafFile(group string, ops []Op) string {
	...
	l(")")
	l("")
	l("// Import guards: the generator always emits the same import block regardless of")
	l("// which operations end up in this file. Not every file will use every package")
	l("// (e.g. strings and io are only needed when an operation has a request body).")
	l("// These blank assignments keep all imports alive without any runtime cost.")
	l("var (")
